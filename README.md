...
| ディレクトリ | 説明 |
| :-- | :-- |
| `shared/` | AI エージェント共通ルール — Claude Code / Codex 双方が参照する正本 |
...
| `vim/` | vim 設定 (最小構成) |
| `claude/` | Claude Code 設定 (`~/.claude/`) — `CLAUDE.md` (shared への `@` import) と skills |
| `codex/` | Codex 設定 (`~/.codex/`) — `AGENTS.md` (shared への symlink) と MCP 設定 |
...
- `shared/agent-guidance.md` を共通ルールの正本として扱う
- `claude/.claude/CLAUDE.md` は `@` import で `shared/agent-guidance.md` を取り込む (相対パス、クロスプラットフォーム対応)
- `codex/.codex/AGENTS.md` は `shared/agent-guidance.md` への symlink
